...
const PaymentModal = ({ element, index }) => {
  ...
  // };
  const handleCopyToClipboard = (text, accountNumber) => {
    clipboardCopy(text) // Use clipboard-copy to copy text to clipboard
      .then(() => {
        console.log("Text copied to clipboard:", text);
  ...
  return (
    <div
      id={index}
      className="overflow-auto w-full  flex flex-col justify-center items-center text-center"
    >
      ...
      <div className=" flex flex-col items-center text-base lg:text-sm">
        <div>
          Send the sum of{" "}
          <span className=" text-lexAccent "> {element.coursePrice} </span> to
          any of the following account numbers and send a proof of payment on
          WhatsApp by clicking this icon:
        </div>
      ...
      <p
        ...
        className={
          copiedAccount === "2136383824" ? "text-lexAccent mt-2" : "mt-2"
        }
      >
      ...
      <p
        ...
        className={
          copiedAccount === "9061872075" ? "text-lexAccent mt-2" : "mt-2"
        }
      >
        {copiedAccount === "9061872075" ? "Copied!" : "9061872075 - Opay"}
      </p>
      <p className="my-2 text-xs ">Click Acc No to copy</p>
